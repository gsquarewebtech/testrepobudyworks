...
function smp_remove_podcasting_tab_from_settings( $tabs ) {
	if ( ! array_key_exists( 'podcast', $tabs ) ) {
		return $tabs;
	}

	$keys = array_keys( $tabs );

	$keys[ array_search( 'podcast', $keys ) ] = 'podcasting';
...
function smp_render_licensing_field( $value, $option_value, $description, $tooltip_html, $custom_attributes ) {
	...
	// Force check.
	if ( null === $is_valid && $option_value ) {
		$is_valid = \SMP\Plugin::instance()->licensing_manager->recheck_license( $option_value );
